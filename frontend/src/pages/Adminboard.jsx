...
const Buyers = () => {
  ...
  const [data, setData] = useState([
    ...
    },
    {
      ...
      comments:
        "I have been looking for this model, finally found at affordable cost.",
    },
    {
      id: 3,
  ...
  useEffect(() => {
    axios
      .get("https://car-store-app-api.vercel.app/api/get-filess")
      .then((res) => {
  ...
  const handleVerify = (buyerId, carType, carId, formId, isAdmin) => {
    const confirm = window.confirm(
      "Are you sure? Request will be sent to the car owner."
    );
    if (confirm) {
      axios
        .post("https://car-store-app-api.vercel.app/api/admin-accept", {
          buyerId,
          carType,
          carId,
          formId,
  ...
  const handleCancel = (buyerId, carType, carId, formId) => {
    ...
    if (confirm) {
      axios
        .post("https://car-store-app-api.vercel.app/api/admin-cancel", {
          buyerId,
          carType,
          carId,
          formId,
  ...
  //   const baseUrl = "https://car-store-app-api.vercel.app/";
  ...
  const pendingRequests = buyingFormData.filter(
    (buyer) => buyer.adminVerified === "pending"
  ).length;
              ...
              <tbody>
                {buyingFormData.map((buyer) => (
                  <tr key={buyer.id}>
                    ...
                    <td>{buyer.phone}</td>
                    <td>
                      {buyer.houseNo}, {buyer.streetAddress},{buyer.city}
                      <br />
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => handleViewDocument(buyer.pdf)}
                      >
                        View Document
                      </button>
                    </td>
